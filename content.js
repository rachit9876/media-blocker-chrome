...
(function () {
  ...
  // SVG Pixelation Filter Injection
  const PIXELATE_SVG = `
    <svg id="__mb_svg_filters__" width="0" height="0" style="position:absolute;z-index:-1;">
      <filter id="mb-pixelate-filter">
  ...
  const MASTER_CSS = `
    ...
    ${prefix(':root', IMG_ALL)}, ${prefix(':root', VID_SELECTORS)} { will-change: filter, opacity; }

    ${prefix(':root[data-mb-target-img="true"]', IMG_ALL)},
    ${prefix(':root[data-mb-target-vid="true"]', VID_SELECTORS)} {
      filter: var(--mb-filter-func) grayscale(var(--mb-grayscale)) invert(var(--mb-invert)) hue-rotate(var(--mb-hue)) !important;
      opacity: var(--mb-opacity) !important;
      transition: filter 0.4s cubic-bezier(0.4, 0, 0.2, 1), opacity 0.4s ease !important;
    }

    ${prefix(':root[data-mb-target-vid="true"]', VID_SELECTORS)} { transform: translateZ(0); }
    ...
    ${prefix(':root[data-mb-target-vid="true"][data-mb-hover="true"]', VID_SELECTORS).split(',').map(s => `${s.trim()}:hover`).join(', ')} {
      --mb-filter-func: blur(0px) !important; --mb-grayscale: 0% !important; --mb-invert: 0 !important; --mb-hue: 0deg !important; --mb-opacity: 1 !important;
    }
    ...
    /* ALT TEXT REPLACEMENT PLACEHOLDERS */
    ${prefix(':root[data-mb-target-img="true"][data-mb-block="true"]', 'img')} {
      position: relative !important; visibility: hidden !important;
    }
    ${prefix(':root[data-mb-target-img="true"][data-mb-block="true"]', 'img')}::after {
      content: attr(alt) " (Media Blocked)" !important;
      visibility: visible !important; position: absolute !important; top: 0 !important; left: 0 !important;
      width: 100% !important; height: 100% !important;
      background: #1a1a1f !important; color: #a0a0b0 !important;
      font-size: 13px !important; font-family: sans-serif !important;
      display: flex !important; align-items: center !important; justify-content: center !important;
      text-align: center !important; padding: 8px !important; box-sizing: border-box !important;
      border: 1px dashed #3a3a4a !important; overflow: hidden !important; text-overflow: ellipsis !important;
    }
    ${prefix(':root[data-mb-target-vid="true"][data-mb-block="true"]', VID_SELECTORS)} { pointer-events: none !important; }
    ${prefix(':root[data-mb-target-img="true"][data-mb-block="true"]', BG_SELECTORS)} { background-image: none !important; }
  `;

  function injectMasterStyle() {
    // Safely inject SVG Filters for Pixelation
    if (!document.getElementById("__mb_svg_filters__")) {
      const svgContainer = document.createElement("div");
      svgContainer.innerHTML = PIXELATE_SVG;
      (document.body || document.documentElement).appendChild(svgContainer);
    }
    
    // Safely inject CSS Master Rules
  ...
  ['contextmenu', 'copy', 'paste', 'selectstart', 'dragstart', 'mousedown', 'mouseup'].forEach(evt => {
      window.addEventListener(evt, function(e) { if (isForceRightClickOn) { e.stopPropagation(); } }, true);
  });
  ...
  // --- VIDEO AUTOPLAY & AUTO-MUTE OVERRIDE ---
  let vidAutoplayPrev = false;
  let vidAutoMute = false;

  function processVideoNode(v) {
    if (vidAutoplayPrev) {
      if (v.hasAttribute('autoplay')) v.removeAttribute('autoplay');
      if (!v.paused && !v.__mbPaused) { v.pause(); v.__mbPaused = true; }
    }
    if (vidAutoMute) v.muted = true;
  }

  const videoObserver = new MutationObserver((mutations) => {
    if (!vidAutoplayPrev && !vidAutoMute) return;
    mutations.forEach(m => {
      m.addedNodes.forEach(node => {
        if (node.tagName === 'VIDEO') processVideoNode(node);
        else if (node.querySelectorAll) node.querySelectorAll('video').forEach(processVideoNode);
      });
    });
  });

  function triggerVideoProcessing() {
    if (vidAutoplayPrev || vidAutoMute) {
      document.querySelectorAll('video').forEach(processVideoNode);
  ...
  function attachStableVolume(mediaEl) {
    ...
    try {
      ...
      lowEQ.type = "lowshelf"; lowEQ.frequency.value = 250;
      
      const midEQ = audioCtx.createBiquadFilter();
      midEQ.type = "peaking"; midEQ.frequency.value = 2000; midEQ.Q.value = 1.0;

      const compressor = audioCtx.createDynamicsCompressor();
      compressor.threshold.value = -20; compressor.knee.value = 20;       
      compressor.ratio.value = 4; compressor.attack.value = 0.005; compressor.release.value = 0.1;   
  ...
  function updateEQNodes(nodes) {
    if (!nodes) return;
    if (audioEqMode === 'stable') { nodes.lowEQ.gain.value = 0; nodes.midEQ.gain.value = 0; }
    else if (audioEqMode === 'dialogue') { nodes.lowEQ.gain.value = -6; nodes.midEQ.gain.value = 5; }
    else if (audioEqMode === 'bass_cut') { nodes.lowEQ.gain.value = -12; nodes.midEQ.gain.value = 0; }
  }
  ...
  document.addEventListener('play', (e) => {
    if (e.target.tagName === 'VIDEO' || e.target.tagName === 'AUDIO') {
      ...
      if (isStableVolumeOn && audioCtx && audioCtx.state === 'suspended') audioCtx.resume();
    }
  }, true);
  // ------------------------------------------
  ...
  function updateVisualFilter() {
    ...
    if (root.getAttribute("data-mb-blur") !== "true") {
       root.style.setProperty("--mb-filter-func", "blur(0px)");
       return;
    }
    if (currentBlurMode === "pixelate") {
      root.style.setProperty("--mb-filter-func", "url(#mb-pixelate-filter)");
    } else {
      root.style.setProperty("--mb-filter-func", `blur(${currentBlurVal}px)`);
    }
  }
  ...
  function showLockScreen() {
    ...
    ['click', 'mousedown', 'keydown', 'wheel', 'contextmenu'].forEach(evt => overlay.addEventListener(evt, e => e.stopPropagation(), true));
  }
